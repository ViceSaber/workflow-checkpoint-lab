...
function isCompactLayout(projectDir) {
  if (process.env.AI_WORKFLOW_LAYOUT === 'compact') return true;
  if (process.env.AI_WORKFLOW_LAYOUT === 'legacy') return false;
  return fs.existsSync(path.join(projectDir, '.ai-workflow', 'config.yml'));
}

function workflowStateDir(projectDir) {
  if (process.env.AI_WORKFLOW_STATE_DIR) {
    return path.resolve(projectDir, process.env.AI_WORKFLOW_STATE_DIR);
  }
  return isCompactLayout(projectDir)
    ? path.join(projectDir, '.ai-workflow', 'state')
    : path.join(projectDir, '.claude');
}

function workflowPath(projectDir, name) {
  return path.join(workflowStateDir(projectDir), name);
}

function projectRootFromHookDir(hookDir) {
  const twoUp = path.resolve(hookDir, '..', '..');
  return path.basename(twoUp) === '.ai-workflow'
    ? path.dirname(twoUp)
    : twoUp;
}
